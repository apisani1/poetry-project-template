...
from tests.project_structure import docs_files, github_workflow_files, expected_files, custom_context
...
def test_documentation_structure(default_project):
    ...
    for path in docs_files:
        assert os.path.exists(path_in_output(default_project, path)), f"Missing documentation file or directory: {path}"
...
def test_custom_project_parameters(custom_project):
    ...
    assert custom_context['project_name'] in pyproject_content in pyproject_content, \
    ...
    assert custom_context['description'] in readme_content, \
